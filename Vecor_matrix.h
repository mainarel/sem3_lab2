...
ArraySequence<ArraySequence<string>*>* new_matrix() {
	...
	Print(matrix->Get(0), matrix->Get(0)->GetLength() - 1);
	Print(matrix->Get(1), matrix->Get(1)->GetLength() - 1);
	Print(matrix->Get(2), matrix->Get(2)->GetLength() - 1);
//	cout << matrix->GetLength()-1 << endl;
	return matrix;
	}


IDictionary<int, string>* make_sprace_matrix(ArraySequence<ArraySequence<string>*>* matrix) {
	IDictionary<int, string>* IDic = new IDictionary<int, string>();
	//цикл по столбцам
	for (long i = 0; i < matrix->GetLength() - 1; i++) {
		//цикл по строкам
		for (long j=0; j< (matrix->Get(i))->GetLength()-1; j++)
			if (matrix->Get(i)->Get(j) != "0") {
				IDic->Add(i, matrix->Get(i)->Get(j));
			}
	}
	return IDic;
}
...
IDictionary<int, TValue>* make_sprace_vector(ArraySequence<T>* vector) {
	...
	for (long i = 0; i < vector->GetLength()-1; i++) {
		if (vector->Get(i)!=0) {
